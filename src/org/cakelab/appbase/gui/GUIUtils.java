...
public class GUIUtils {
	 ...
	 * 
	 * @param frame The window / {@link JFrame} which is centred on the screen.
	 */
	...
	public static void makeGrid(Container parent, int rows, int cols, int initialX, int initialY, int xPad, int yPad) {
		...
		for (int col = 0; col < cols; col++) {
			Spring maxWidthSpring = layout.getConstraints(parent.getComponent(col)).getWidth();
			Spring maxHeightSpring = layout.getConstraints(parent.getComponent(col)).getHeight();
			for (int row = 0; row < rows; row++) {
				SpringLayout.Constraints cons = layout.getConstraints(parent.getComponent(row * cols + col));
	...
	/**
	 * Sets the specified cursor type for the specified component.
	 * 
	 * For cursor types see {@link java.awt.Cursor}.
	 * 
	 * @param component
	 * @param type
	 */
	...
	public static void adjustToDesktopSize(Dimension dim) {
		...
		if (desktopSize.height < dim.height)
			dim.height = desktopSize.height;
		if (desktopSize.width < dim.width)
